...
const ProjectSidebar = ({ activeTab, setActiveTab, userRole, isOpen, onClose }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar / Drawer */}
            <motion.div
                // Removed conflicting Framer Motion props to allow CSS to handle responsive visibility
                className={`
                    fixed lg:sticky top-0 left-0 h-screen z-50
                    w-72 flex-shrink-0 
                    dark:bg-[#0A101F]/95 bg-white/95 backdrop-blur-xl 
                    border-r dark:border-white/5 border-slate-200 
                    flex flex-col shadow-2xl lg:shadow-none
                    transition-transform duration-300 ease-in-out
                    ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
                `}
            >
                <div className="p-6 border-b dark:border-white/5 border-slate-100 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="overflow-hidden">
                            ...
                            <p className="text-xs text-cyan-400 font-mono tracking-wider">SECURE CONNECTION</p>
                        </div>
                    </div>
                ...
                <div className="p-4">
                    ...
                    >
                        ...
                        <span className="font-medium text-sm">Return to Base</span>
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1 custom-scrollbar">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <button
                                key={item.id}
                                onClick={() => { setActiveTab(item.id); onClose(); }} // Close on selection for mobile
                                className={`
                            ...
                            >
                                ...
                                <span className={`font-medium text-sm relative z-10 ${isActive ? 'text-white' : ''}`}>
                                    {item.label}
                                </span>
                                ...
                                {isActive && (
                                    <motion.div
                                        initial={{ scale: 0 }}
                                        animate={{ scale: 1 }}
                                        className="ml-auto w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.8)] relative z-10"
                                    />
                                )}
                            </button>
                        );
                ...
                <div className="p-4 border-t dark:border-white/5 border-slate-200">
                    <div className="p-4 bg-gradient-to-br from-cyan-900/10 to-blue-900/10 rounded-2xl border border-cyan-500/10 text-center relative overflow-hidden">
